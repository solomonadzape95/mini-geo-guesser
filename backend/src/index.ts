...
app.post("/games/save", quickAuthMiddleware, async (c) => {
	...
	try {
		...
		/////////////////////////////////////////////////////////
		if (score === 3 && user.primaryAddress) {
			...
			const tokenURI = `https://mini-geo-guesser-a8hd.vercel.app/badges/geo-champion.json`;
			...
			try {
				...
				await supabase.from("user_badges").insert({
					badgeID: 1, // update to badge ID logic
					userID: user.profileId!,
				});

				console.log("Badge NFT minted successfully.");
			} catch (err) {
